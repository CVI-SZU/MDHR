...
from model.model_DISFA import MDHR
from utils.utils import *
from dataset.dataset import video_DISFA_val
import argparse

def get_dataloader(conf):
    print('==> Preparing data...')

    valset = video_DISFA_val(root_path=conf.dataset_path, length=conf.length, fold=conf.fold, transform=image_test(crop_size=conf.crop_size), crop_size=conf.crop_size)
...
def main(conf):
    ...
    torch.save(net.state_dict(), os.path.join('./result', 'best_model.pth'))
...
if __name__ == '__main__':
    ...
    # Datasets
    parser.add_argument('--dataset', default="DISFA", type=str, help="experiment dataset: BP4D / DISFA")
    parser.add_argument('--dataset_path', default='/path/to/DISFA_dataset/', type=str, help="root path to dataset")
    ...
    parser.add_argument('--au_num', default=8, type=int, help='number of AUs')
    ...
    parser.add_argument('--resume', default='', type=str, metavar='path', help='path to latest checkpoint')  #/path/to/best_model_fold1.pth
