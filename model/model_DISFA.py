...
from .utils import calculate_adjmask_disfa
...
class NodeClassifier(nn.Module):
    def __init__(self, class_num=8, in_channels=512):
...
class LocalRelationshipModeling(nn.Module):
    def __init__(self, num_classes=8, in_channels=512, out_channels=512):
        super(LocalRelationshipModeling, self).__init__()
        self.num_classes = num_classes
        self.in_channels = in_channels
        self.out_channels = out_channels

        self.sub_class = [8, 4, 8]
        ...
        self.down_fc = nn.Linear(self.in_channels, self.sub_class[2])
    ...
    def forward(self,x):
        ...
        down_pred = self.down_fc(x_down.mean(dim=1))
        ...
        AU9 = self.class_linears[4](x_mid).unsqueeze(1)
        AU12 = self.class_linears[5](x_down).unsqueeze(1)
        AU25 = self.class_linears[6](x_down).unsqueeze(1)
        AU26 = self.class_linears[7](x_down).unsqueeze(1)

        AU_node = [AU1, AU2, AU4, AU6, AU9, AU12, AU25, AU26]

        f_v = torch.cat(AU_node, dim=1)
        f_v = f_v.mean(dim=-2)

        return up_pred, mid_pred, down_pred, f_v
...
class CrossregionRelationshipModeling(nn.Module):
    ...
    def forward(self, x, adj_mask):
        adj = torch.tensor([[0, 0, 0, 1, 1, 1, 1, 1], #AU1
                            [0, 0, 0, 1, 1, 1, 1, 1], #AU2
                            [0, 0, 0, 1, 1, 1, 1, 1], #AU4
                            [1, 1, 1, 0, 0, 1, 1, 1], #AU6
                            [1, 1, 1, 0, 0, 1, 1, 1], #AU9
                            [1, 1, 1, 1, 1, 0, 0, 0], #AU12
                            [1, 1, 1, 1, 1, 0, 0, 0], #AU25
                            [1, 1, 1, 1, 1, 0, 0, 0]]) #AU26
...
class MDHR(nn.Module):
    def __init__(self, dataset='DISFA', num_classes=8, backbone='resnet', k=5):
    ...
    def forward(self, x):
        ...
        up_pred, mid_pred, down_pred, x = self.LRM(x)
        adj_mask = calculate_adjmask_disfa(up_pred.detach(), mid_pred.detach(), down_pred.detach())
        x = self.CRM(x, adj_mask)
        x = self.TCN(x, B, T)
        x = self.node_classifier(x)
        return up_pred, mid_pred, down_pred, x
