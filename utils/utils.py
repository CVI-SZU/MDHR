...
def adjust_learning_rate(optimizer, epoch, epochs, init_lr, iteration, num_iter, warmup_epoch):
    ...
    if epoch < warmup_epoch:
        lr = init_lr * current_iter / max_warmup_iter
    else:
        lr = init_lr * (1 + cos(pi * (current_iter-max_warmup_iter) / (max_iter-max_warmup_iter))) / 2
...
class image_train(object):
    ...
    def __call__(self, img, offset_x, offset_y, flip, brigtness, contrast, saturation):
        ...
        transform = transforms.Compose([
            ...
            SetFlip(flip),
            ColorJitter(brightness=brigtness, contrast=contrast, saturation=saturation),
            transforms.ToTensor(),
            normalize
        ])
        img = transform(img)
