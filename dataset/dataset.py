...
class video_DISFA_train(Dataset):
    def __init__(self, root_path, length=16, fold=1, transform=None, crop_size=224, loader=default_loader):
        self._root_path = root_path
        self._transform = transform
        self.crop_size = crop_size
        ...
        self.length = length
        self.fold = fold
        self.img_folder_path = os.path.join(self._root_path, 'img_o')
        self.data = []
        # img
        train_image_list_path = os.path.join(self._root_path, 'DISFA_train_path_fold'+str(self.fold)+'.txt')
        train_image_list = open(train_image_list_path).readlines()
...
class video_DISFA_val(Dataset):
    def __init__(self, root_path, length=16, fold=1, transform=None, crop_size=224, loader=default_loader):
        ...
        self.data = []
        # img
        val_image_list_path = os.path.join(self._root_path, 'DISFA_test_path_fold'+str(self.fold)+'.txt')
        val_image_list = open(val_image_list_path).readlines()
        ...
        for i in range(len(val_label_list)):
            ...
            if frame_num == self.length:
                ...
                if val_image_list[i].split('/')[0] != val_image_list[i+1].split('/')[0]:
                    while frame_num < self.length:
                        img_path.append(img_path[-1])        #      padding with the last image
                        img_label.append(-np.ones(8))       #      label padding with -1
                        img_sub_label.append(img_sub_label[-1])   #      sub_label padding with -1
                        frame_num += 1
                    video_dic['img_path'] = img_path
                    video_dic['img_label'] = np.array(img_label)
                    video_dic['img_sub_label'] = np.array(img_sub_label)
